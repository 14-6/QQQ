...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#121212] text-white font-sans overflow-x-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <header className="relative w-full min-h-[500px] sm:min-h-[550px] lg:h-[85vh] overflow-hidden flex items-center justify-center text-white py-12 sm:py-0">

        {/* الشريط العلوي */}
        <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 z-50 max-w-7xl mx-auto flex items-center justify-between">
          
          <button
            onClick={toggleLanguage}
            className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs font-semibold backdrop-blur transition-all duration-300 active:scale-95 cursor-pointer border border-white/20 bg-black/40 hover:bg-black/60 text-amber-400"
          >
            <Globe className="w-3.5 h-3.5 text-amber-400" />
            <span>{lang === 'ar' ? 'English' : 'عربي'}</span>
          </button>

          <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full border border-amber-400/60 bg-black/50 backdrop-blur-md flex items-center justify-center shadow-lg hover:scale-105 transition-transform duration-300">
        ...
        <div className="relative z-30 text-center px-4 max-w-4xl mx-auto flex flex-col items-center justify-center pt-16 sm:pt-12">
          ...
          {/* بطاقة الإحصائيات مع تجاوب الشاشة */}
          <div className="flex items-center justify-around w-full max-w-sm sm:max-w-lg bg-neutral-900/70 backdrop-blur-md border border-neutral-700/50 rounded-2xl p-3 sm:p-4 my-4 text-white shadow-xl">
